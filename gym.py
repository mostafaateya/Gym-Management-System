...
def send_sms_notification(to_phone_number, message1, title1):
    print("PHONE NUMBER", to_phone_number)
    print("MESSAGE", message1)
    print("TITLE", title1)

    messagebox.showinfo(title=title1, message=message1)
...
def check_date():
    ...
    for registration in registrations:
        ...
        if datetime.strptime(end_date, '%Y-%m-%d') < current_date:
            ...
            sms_message="Your gym membership has expired. Renew your subscription to continue accessing ALX GYM."
            send_sms_notification(contact_no, sms_message, 'Expiration Text')
...
def send_sms_for_expiration():
    ...
    for registration in registrations:
        ...
        if datetime.strptime(end_date, '%Y-%m-%d') == current_date + timedelta(days=3):
            ...
            # Send SMS to the member
            sms_message="Your gym membership will expire in 3 days. Renew your subscription to continue accessing ALX GYM."
            send_sms_notification(contact_no, sms_message, 'Expiring date after 3 days')
...
def create_home_frame(home):
    ...
    income_label=ctk.CTkLabel(income_frame, text="Membership Monthly Income (EGP)", font=("Arial bold", 16))
    ...
    graph_frame2.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")

    # Create a small rectangular label for the income report
    income_label2=ctk.CTkLabel(graph_frame2, text="Gymers Monthly Income (EGP)", font=("Arial bold", 16))
...
def update_income_report(root, ax, canvas):
    ...
    # Retrieve monthly member count and subscription plan
    cursor_members.execute("""
        SELECT strftime('%Y-%m', start_date) as month, subscription_plan, COUNT(*)
        FROM registration
    ...
    # Define subscription plan rates
    subscription_rates = {
        "Weekly": 350,
        "Monthly": 700,
        "Yearly": 3000
    ...
    # Process member data to calculate total income based on subscription plans
    merged_data = {}
    for month, plan, count in members_data:
        rate = subscription_rates.get(plan, 0)  # Get the rate for the plan, default to 0 if not found
        income = count * rate

        if month not in merged_data:
            merged_data[month] = {}

        merged_data[month][plan] = income

    # Example: If you need to sum up all incomes for each month
    for month, income_data in merged_data.items():
        total_income = sum(income_data.values())
        merged_data[month]['total_income'] = total_income
    ...
    for month, data in merged_data.items():
        total_income = sum(data.get(plan, 0) for plan in ['Weekly', 'Monthly', 'Yearly'])
        months.append(month)
        total_incomes.append(total_income)

    # Plot the monthly income report with inverted colors
    ax.clear()
    members_bar=ax.bar(months, total_incomes, color='green', alpha=0.7, label='Members')
    ...
    for bar, members_income in zip(members_bar, total_incomes):
...
def update_visitors_income_report(root, ax, canvas):
    ...
    cursor_visitors.execute("""
        SELECT substr(time_start, 1, 7) as month, COUNT(*)
        FROM visitors
    ...
    visitor_rate = 50  # Income per visitor (e.g., $50 per visitor)

    for month, visitors_count in visitors_data:
        total_income = visitors_count * visitor_rate
        merged_data[month] = total_income
    
    months, visitor_incomes = zip(*merged_data.items())
    ...
    for bar, visitors_income in zip(visitor_bar, visitor_incomes):
        ax.text(bar.get_x() + bar.get_width() / 2, visitors_income,
                f'{visitors_income} EGP', ha='center', va='bottom', color='black', fontweight='bold')
...
class RegistrationFrame(ctk.CTkFrame):
    def __init__(self, master, data=None, **kwargs):
        ...
        label=ctk.CTkLabel(self, text="ALX Gym Membership Registration", font=("Arial bold", 26))
        ...
        nationalities_list=["Select Nationality", "Egyption", "American", "Chinese", "Japanese", "Korean", "Other"]
        ...
        contact_no_entry=ctk.CTkEntry(contact_frame, placeholder_text="+20 1234567890")
        ...
        emergency_contact_entry=ctk.CTkEntry(contact_frame, placeholder_text="+20 1234567890")
        ...
        self.subscription_id_entry=ctk.CTkEntry(subscription_frame, placeholder_text="AG-XXXXXXXX")
        ...
        outer_frame2=ctk.CTkFrame(outer_frame)
        outer_frame2.pack(pady=20, padx=10)

        # Create a "Register" button
        register_button=ctk.CTkButton(outer_frame2, text="Register", fg_color="Green",
                                      text_color=("gray10", "gray90"),
                                      hover_color=("green3", "green4"),
                                      command=self.register_subscription)
        #register_button.pack(padx=(10,5), pady=20)
        register_button.pack(side="left", padx=(10,5), pady=20)

        # create a  back button to return to the previous frame
        back_button=ctk.CTkButton(outer_frame2, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
                                  hover_color=("red3", "red4"), command=self.back_button_event)
        #back_button.place(x=450, y=550)
        back_button.pack(side="right", padx=(5,10), pady=20)
    ...
    def set_subscription_id():
        ...
        result_str="AG-" + ''.join(random.choice(letters) for i in range(8))
        return result_str

    def send_sms(self, to_phone_number, message1, title1):
    ...
    def register_subscription(self):
        ...
        file_path=os.path.join(folder_path, f"ALXG_{last_name}.png")
        ...
        sms_message=f"Hello {first_name}!, You have Successfully Subscribed for {subscription_plan} Plan. Subscription ID:{subscription_id}. Start Date: {start_date} End Date: {end_date}, - ALX GYM"
        self.send_sms(formatted_contact_no, sms_message, 'Subscripsion Text')
...
class ViewFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        view_button_frame.pack(pady=10, padx=10, fill="x")

        # Create an "Edit" button in the second column
        view_button=ctk.CTkButton(view_button_frame, text="View", command=self.edit_record)
        view_button.pack(side="left",padx=(10,5), pady=10)

        # Create a "Back" button to return to the previous frame
        back_button=ctk.CTkButton(view_button_frame, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
                                  hover_color=("red3", "red4"), command=self.back_button_event)
        back_button.pack(side="right", padx=(5,10), pady=10)
...
class EditForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        qr_code_path=os.path.join("templates/member_qrcodes", f"ALXG_{self.member_data[3]}.png")
    ...
    def download_qr_code(self):
        ...
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"ALXG_{self.member_data[3]}.png")
    ...
    def update_record(self):
        ...
        try:
            ...
            if end_date == current_date + timedelta(days=3):
                ...
                sms_message="Your gym membership will expire in 3 days. Renew your subscription to continue accessing ALX GYM."
                self.send_sms(formatted_contact_no, sms_message, 'Expiring date after 3 days')
            ...
            if end_date <= current_date:
                ...
                formatted_contact_no=self.entry_fields[8].get()
                sms_message="Your gym membership has expired. Renew your subscription to continue accessing ALX GYM."
                self.send_sms(formatted_contact_no, sms_message, 'Expiration Text')
...
class RenewSubscriptionFrame(ctk.CTkToplevel):
    ...
    def renew_membership(self):
        ...
        try:
            ...
            if end_date <= datetime.now().strftime('%Y-%m-%d'):
                ...
                self.send_sms_expiration(self.member_data[1], start_date, end_date, 'Expiration check')
            ...
            self.send_sms(formatted_contact_no, sms_message, 'Renewal Text')
    ...
    def send_sms_expiration(self, contact_no, start_date, end_date, title1):
        ...
        self.send_sms(formatted_contact_no, sms_message, title1)
            ...
            with sqlite3.connect('SQLite db/attendance_records.db') as conn_attendance:
                ...
                if attendance_type == "Time In":
                    ...
                    self.send_sms(contact_no,
                                  f"Hello {first_name}!, You have Successfully Time In. Subscription ID:{subscription_id}. Time In: {current_datetime}, - ALX GYM",'Welcom to the floor')
                    ...
                    self.send_sms(contact_no,
                                  f"Hello {first_name}!, Thank you for coming. See you again! Subscription ID:{subscription_id}. Time Out: {current_datetime}, - ALX GYM", 'Goodbye')
...
class RegistrationEquipment(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        outer_frame1=ctk.CTkFrame(outer_frame)
        outer_frame1.pack(padx=10, pady=10)

        # Create a "Register" button
        register_button=ctk.CTkButton(outer_frame1, text="Register", fg_color="Green",
                                      text_color=("gray10", "gray90"),
                                      hover_color=("green3", "green4"),
                                      command=self.register_equipment_info)
        register_button.pack(side="left", padx=(10,5), pady=15)

        # create a back button to return to the previous frame
        back_button=ctk.CTkButton(outer_frame1, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
                                  hover_color=("red3", "red4"), command=self.back_button_event)
        #back_button.place(x=450, y=550)
        back_button.pack(side="right", padx=(5,10), pady=15)
...
class TrainerFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        outer_frame1=ctk.CTkFrame(outer_frame)
        outer_frame1.pack(pady=20, padx=10)
        ...
        register_button.pack(side="left", padx=(10,5), pady=5)

        # Create a "Back" button to return to the previous frame
        back_button=ctk.CTkButton(outer_frame1, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
                                  hover_color=("red3", "red4"), command=self.back_button_event)
        #back_button.pack(pady=5, side=tk.TOP)
        back_button.pack(side="right", padx=(5,10), pady=5)
    ...
    def register_subscription(self):
        ...
        # Specify the file path to save the QR code in the folder
        file_path=os.path.join(folder_path, f"ALXG_trainer_{last_name}.png")
        qr_img.save(file_path)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        sms_message=f"Hello {first_name}!, You are registered as a Trainer in ALX Gym."
        self.send_sms(formatted_contact_no, sms_message, 'Welcom to the family')
...
class EditTrainerForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        # Display the qr code from the member_qrcodes folder based on the last name of the member
        qr_code_path=os.path.join("templates/trainer_qrcodes", f"ALXG_trainer_{self.trainer_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)
        qr_code_image=qr_code_image.resize((200, 200), Image.LANCZOS)
        qr_code_image=ImageTk.PhotoImage(qr_code_image)
        qr_code_label=ctk.CTkLabel(qr_code_frame, text="", image=qr_code_image)
    ...
    def download_qr_code(self):
        ...
        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"ALXG_trainer_{self.trainer_data[3]}.png")
        qr_code_image.save(save_path)
                ...
                if status and status[0] in ['Active', 'On Leave']:
                    ...
                    with sqlite3.connect('SQLite db/trainer_attendance_records.db') as attendance_conn:
                        ...
                        if attendance_type == "Time In":
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time In. Time In: {current_datetime}, - ALX GYM", 'Welcome to the floor')
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time Out. Time In: {current_datetime}, - ALX GYM", 'Goodbye')
...
class LogbookFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        contact_no_entry=ctk.CTkEntry(personal_info_frame, placeholder_text="+20 1234567890")
    ...
    def attend_log(self):
        ...
        try:
            ...
            self.send_sms(contact_no,
                          f"Hello {first_name}!, Thank You for attending ALX GYM. Time In: {current_time}", 'Welcome to the floor')
...
class RegisterEmployeeFrame(ctk.CTkFrame):
    ...
    def register_employees(self):
        ...
        # Specify the file path to save the QR code in the folder
        file_path=os.path.join(folder_path, f"ALXG_employee_{last_name}.png")
        qr_img.save(file_path)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        sms_message=f"Hello {first_name}!, You are registered as an Employee of ALX Gym."
        self.send_sms(formatted_contact_no, sms_message, 'Welcome to the family')
...
class EditEmployeeForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        # Display the qr code from the member_qrcodes folder based on the last name of the member
        qr_code_path=os.path.join("templates/employee_qrcodes", f"ALXG_employee_{self.employee_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)
        qr_code_image=qr_code_image.resize((200, 200), Image.LANCZOS)
        qr_code_image=ImageTk.PhotoImage(qr_code_image)
        qr_code_label=ctk.CTkLabel(qr_code_frame, text="", image=qr_code_image)
    ...
    def download_qr_code(self):
        ...
        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"ALXG_employee_{self.employee_data[3]}.png")
        qr_code_image.save(save_path)
...
class RecordsAttendanceFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        # back button
        back_button=ctk.CTkButton(table_frame, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
                                  hover_color=("red3", "red4"), command=self.back_button_event)
        back_button.pack(pady=5, side=tk.TOP)
...
def register(full_name, contact_no, username, password, contact_no_entry, full_name_entry, username_entry,
    ...
    try:
        ...
        if existing_user:
            ...
            send_sms(contact_no, f"Hello {full_name}!, You have Successfully Registered as user. - ALX GYM", 'Welcome to the family')
            conn.commit()
...
def send_sms(to_phone_number, message1, title1):
    print("PHONE NUMBER", to_phone_number)
    print("MESSAGE", message1)
    print("TITLE", title1)
...
def forgot_password():
    ...
    while True:
        ...
        if user_phone_number is None:
            ...
            if registered_user:
                send_sms(user_phone_number, f'Your OTP is: {otp}','OTP Text')
                    ...
                    if new_username and new_password:
                        if is_valid_password(new_password):
                            ...
                            send_sms(user_phone_number,
                                     f'Your username and password have been reset. Username: {new_username}. Password:{new_password}.','Username and password reset')
                            conn.commit()
                            conn.close()
